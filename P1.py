    ...
    if event.startswith('11:42'):  # évenement qui commence par "11:42" (ils commencent tous par 11:42)
        ...
        if len(texte) > 2:  #
            options1 = texte[2].split(
                "]")  # On part du premier "[" et on a texte [2] pour arriver à ce qu'on souhaite récupérer
        ...
        if len(texte) > 2:  # vérifier le nombre de partie (split au crochet)
            length1 = texte[2].split(
                " ")  # on part du premier "[" et ce qu'on recherche est bien dans texte [2]. On split à l'espace pour avoir que le nombre
            length = length1[
                2]  # On veut bien le "2" pour avoir que le nombre (il y a une partie avant l'espace + une autre après).
        # length (sans option)
        texte = event.split(",")
        if len(texte) > 3:
            if texte[3].startswith(" length"):  # Si ça commence par " length" et on recherche dans le texte [3]
                ...
                length = length1[
                    2]  ##On veut bien le "2" pour avoir que le nombre (texte [1] avant l'espace c'est le mot "length").
                length = length.replace(characters,
                                        "")  # remplacement du "characters" en " " (pour éviter que le tableur écrit sous forme de date)
